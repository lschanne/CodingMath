window.onload = function() {
    ...
    function addPoint(x, y, additionalParams) {
        ...
        points.push({
            ...
            static: additionalParams.static || false,
        });
    }
    ...
    function updateSticks() {
        for (var i=0; i < sticks.length; i++) {
            ...
            // if both of the points are static, offsetX and offsetY should
            // be 0, so it doesn't matter which block this hits
            // if one of the points is static, then only move the other point
            // if neither point is static, then we can split =the offset
            // between them both
            if (s.p0.static) {
                s.p1.x += offsetX * 2;
                s.p1.y += offsetY * 2;
